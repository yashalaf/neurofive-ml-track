...
Took the Week 4 churn pipeline (`churn_pipeline.joblib` — Logistic Regression inside a scikit-learn `Pipeline` with `StandardScaler` + `OneHotEncoder`, 81.05% accuracy) and turned it into a shareable Streamlit web app.

**App files added to the repo:**
- `app.py` — Streamlit app with input fields for all raw customer attributes (contract type, tenure, monthly/total charges, service subscriptions, etc.), a **Predict** button, and a churn probability display
- `requirements.txt` — dependencies for deployment (`streamlit`, `scikit-learn`, `pandas`, `numpy`, `joblib`)
- `churn_pipeline.joblib` — the saved, fitted pipeline the app loads at runtime
...
**Live app:** _[add your Streamlit Community Cloud URL here after deploying]_
...
5. Copy the live URL it gives you and paste it above
